...
function removeProperties(obj, keys) {
  const copyObj = Object.assign.apply(null, [{}, obj]);
  keys.forEach((key) => {
    if (key in copyObj) {
      delete copyObj[key];
    }
...
function makeWord(lettersObject) {
  const possitions = Object.values(lettersObject).flat;
  const maxPosition = possitions.length ? Math.max(...possitions) : 0;

  const newArr = Array(maxPosition + 1).fill('');
  Object.entries(lettersObject).forEach(([letter, possition]) => {
    possition.forEach((pos) => {
      newArr[pos] = letter;
    });
  });
  return newArr.join('');
